...
impl PreBackend for Box<dyn PreBackend + Send + Sync> {
    fn backend_id(&self) -> BackendId {
        (**self).backend_id()
    }
...
// Allow using &dyn PreBackend as a PreBackend (for server usage with Arc)
impl<T: PreBackend + ?Sized> PreBackend for &T {
    fn backend_id(&self) -> BackendId {
        (**self).backend_id()
    }
