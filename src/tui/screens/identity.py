...
class IdentityScreen(Widget):
        ...
        with Container(id="identity-container"):
            ...
            with Horizontal(id="identity-operations-row"):
                with Vertical(id="create-identity-panel"):
                    ...
                    yield Button("Create Identity", id="create-identity-btn", variant="primary")
                
                with Vertical(id="load-identity-panel"):
                    yield Label("Load Existing Identity")
                    yield Input(placeholder="Identity file path", id="load-identity-path")
    ...
    def setup_identity_table(self) -> None:
        ...
        table.add_rows([
            ("default", "~/.dcypher/default.json", "2024-01-15", "2024-01-20", "Active"),
            ("backup", "~/.dcypher/backup.json", "2024-01-10", "2024-01-18", "Inactive"),
            ("test", "~/.dcypher/test.json", "2024-01-05", "2024-01-12", "Inactive"),
        ])
    ...
    def create_identity_info_panel(self) -> Panel:
        ...
        content.append(f"Version: {self.identity_info.get('version', 'unknown')}\n", style="dim")
        content.append(f"Derivable: {self.identity_info.get('derivable', False)}\n", style="dim")
    ...
    def action_create_identity(self) -> None:
        ...
        try:
            ...
            mnemonic, file_path = KeyManager.create_identity_file(name, identity_dir, False)
            
            self.notify(f"Identity '{name}' created successfully!", severity="information")
            self.notify("Please backup your mnemonic phrase securely!", severity="warning")
    ...
    def load_identity_file(self, file_path: str) -> None:
        ...
        try:
            ...
            self.notify(f"Identity loaded: {identity_path.name}", severity="information")
            self.update_identity_display()
