...
use dcypher_core::pre::PreBackend;
use dcypher_ffi::ed25519;
use dcypher_ffi::liboqs::{pq_keygen, PqAlgorithm};
...
async fn list_identities(ctx: &Context) -> Result<()> {
    ...
    if wallet.data.identities.is_empty() {
        if !ctx.json_output {
            print_info("No identities yet. Create one with: dcypher identity new");
        }
        return Ok(());
    }
