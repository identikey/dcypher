    ...
    def create_dashboard(self) -> Container:
        ...
        with Container(id="dashboard-container"):
            ...
            with Horizontal(id="status-row"):
                yield Static(
                    self.create_status_panel("IDENTITY STATUS", "✓ Loaded", "green"),
                    id="identity-status",
                )
                yield Static(
                    self.create_status_panel("API CONNECTION", "✓ Connected", "green"),
                    id="api-status",
                )
                yield Static(
                    self.create_status_panel(
                        "FILES & SHARES", "Files: 12\nShares: 5", "cyan"
                    ),
                    id="files-status",
                )
        ...
        with Container():
            ...
            with Horizontal():
                ...
                with Vertical():
                    ...
                    yield Button("Recrypt Data")

            # Progress and results
            yield ProgressBar(total=100, progress=75, id="crypto-progress")
            yield Static(
                "Last operation: File encrypted successfully ✓", id="crypto-results"
            )
...
Last Used: Just now"""
    ...
    def action_help(self) -> None:
        ...
        self.notify(
            "dCypher TUI Demo - Use tabs to navigate, Ctrl+C to exit",
