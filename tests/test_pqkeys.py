...
def test_add_and_remove_pq_keys():
    ...
    response = client.post(f"/accounts/{pk_classic_hex}/add-pq-keys", json=add_payload)
    ...
    nonce3 = get_nonce()
    message3 = f"REMOVE-PQ:{pk_classic_hex}:{alg_to_remove}:{nonce3}".encode("utf-8")
...
def test_replace_existing_pq_key():
    ...
    try:
        ...
        # Add the new signer to the dict for cleanup
        all_pq_sks[pk_falcon_new_hex] = (sig_falcon_new, falcon_alg)
        # The old signer for pk_falcon_old_hex is still in all_pq_sks,
        # but the key itself is now in the graveyard.
        # We must remove the old signer from our tracking dict to avoid a double-free,
        # as it's no longer part of the active account keys we manage.
        del all_pq_sks[pk_falcon_old_hex]
...
def test_add_unsupported_pq_key_fails():
...
def test_add_mandatory_pq_key_again_fails():
...
def test_add_and_remove_multiple_pq_keys():
    ...
    add_payload = {
        ...
        "classic_signature": sk_classic.sign(add_msg, hashfunc=hashlib.sha256).hex(),
        "existing_pq_signatures": [
            {
                "public_key": pk_ml_dsa_hex,
                "signature": sig_ml_dsa.sign(add_msg).hex(),
    ...
    remove_msg = (
        f"REMOVE-PQ:{pk_classic_hex}:{':'.join(algs_to_remove)}:{remove_nonce}".encode()
    )
    remove_payload = {
