...
def test_serial_cryptocontext(tmp_path):
    ...
    assert fhe.SerializeToFile(
        str(tmp_path / "cryptocontext.json"), cryptoContext, fhe.JSON
    )
    LOGGER.debug("The cryptocontext has been serialized.")
    assert fhe.SerializeToFile(
        str(tmp_path / "ciphertext1.json"), ciphertext1, fhe.JSON
    )
    ...
    cc, success = fhe.DeserializeCryptoContext(
        str(tmp_path / "cryptocontext.json"), fhe.JSON
    )
    assert success
    ...
    ct1, success = fhe.DeserializeCiphertext(
        str(tmp_path / "ciphertext1.json"), fhe.JSON
    )
    assert success
...
def test_serial_cryptocontext_str(mode):
    ...
    initialPlaintextAddResult = [
        vectorOfInts1[i] + vectorOfInts2[i] + vectorOfInts3[i]
        for i in range(len(vectorOfInts1))
    ]
    initialPlaintextAddResult = cryptoContext.MakePackedPlaintext(
        initialPlaintextAddResult
    )

    # Multiply the values
    initialPlaintextMultResult = [
        vectorOfInts1[i] * vectorOfInts2[i] * vectorOfInts3[i]
        for i in range(len(vectorOfInts1))
    ]
    initialPlaintextMultResult = cryptoContext.MakePackedPlaintext(
        initialPlaintextMultResult
    )
    ...
    cryptoContext.EvalRotateKeyGen(
        keypair.secretKey,
        [VECTOR1_ROTATION, VECTOR2_ROTATION, VECTOR3_ROTATION, VECTOR4_ROTATION],
    )
